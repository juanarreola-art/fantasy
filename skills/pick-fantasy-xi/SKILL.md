...
Primary objective:
Return exactly 11 eligible player IDs from the current daily game board, using a valid formation and a conservative point-maximizing strategy.
...
4. Choose the best valid formation from available candidates.
...
2. For all positions, prior starts and prior appearances are useful positive signals when current data is limited.
3. For FWD and attacking MID, prioritize goal involvement: goals, assists, shots, attacking role, set pieces, and strong team scoring context when available.
4. For DEF and GK, prioritize clean-sheet chance, team defensive strength, and favorable matchup context when available.
5. For GK, also value likely save volume, but do not prefer a heavily pressured goalkeeper over a strong clean-sheet candidate unless the board strongly supports it.
6. Avoid players with high card risk when similar alternatives exist, especially defenders and defensive midfielders.
7. Prefer balanced certainty over celebrity value. A famous player with weak minutes evidence is not automatically better than a reliable starter.
8. If the board includes current tournament or selected-day stats, prefer those over older historical stats.
9. If only prior World Cup stats are available, use them as weak supporting signals, not decisive proof.
10. If venue, host status, or home advantage appears in the board, give a small boost to host nations or home-like conditions. Keep this boost smaller than minutes, role, and matchup quality.

Formation guidance:

1. Default to 1 GK, 4 DEF, 4 MID, 2 FWD when the player pool is balanced.
2. Use 1 GK, 3 DEF, 4 MID, 3 FWD when there are several strong forwards and fewer reliable defenders.
...
7. Host nation or home-like advantage, if present.
